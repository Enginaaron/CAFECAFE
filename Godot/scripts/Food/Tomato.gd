...
@onready var tomatoTimer = $tomatoTimer
@onready var tomatoBar = $tomatoBar
...
var initial_player = null  # Player who placed the tomato
var last_chopping_player = null  # Track who last chopped the tomato

@export var ingredient_name: String = "tomato"
...
func _ready():
	tomatoBar.value = 0
	tomatoTimer.timeout.connect(_on_tomatoTimer_timeout)  # Connect only once
	update_sprite()
...
func _process(_delta):
	if tomatoTimer.time_left > 0:
		var progress = 100 * (1 - (tomatoTimer.time_left / tomatoTimer.wait_time))
		tomatoBar.value = progress

func get_current_player():
	# If we're on the chopping board, return the player that's currently interacting
	if on_chopping_board:
		# Find any player who can interact with the tomato
		var players = get_tree().get_nodes_in_group("players")
		for potential_player in players:
			# Check if this player is facing the tomato and has empty hands
...
func pick_up():
	...
	if player:
		...
		print("tomato picked up by player ", player.player_number)
...
func chop():
	...
	if state == State.WHOLE:
		...
		if not on_chopping_board:
			...
			# Store the initial player who placed the tomato
			initial_player = player
			print("Player ", initial_player.player_number, " placed tomato on chopping board")
			...
			# Move the tomato to the chopping board
		...
		# Update progress bar
		tomatoBar.value = (chop_progress / float(chop_required)) * 100
		...
		if chop_progress >= chop_required:
			...
			print("tomato attached to player ", player.player_number if player else "unknown")
...
func bowl():
	...
	if state == State.CHOPPED:
		...
		tomatoBar.value = 0  # Reset progress
		tomatoBar.visible = true  # Show progress bar
		
		# Calculate packaging time based on player's package speed
		var baseTime = 5.0
		var basetime = player.PACKAGE_SPEED  # Base speed is 5
		tomatoTimer.wait_time = max(1.0, basetime)  # Minimum 1 second
		print("Packaging time: ", tomatoTimer.wait_time, " seconds (Package Speed: ", player.PACKAGE_SPEED, ")")
		
		tomatoTimer.start()  # Start the timer
...
func _on_tomatoTimer_timeout():
	...
	if is_instance_valid(player):
		...
		tomatoBar.visible = false  # Hide bar when packaging is done
		update_sprite()
